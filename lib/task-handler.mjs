...
export async function executeTaskWork(taskId, agent, { gateway, agentRegistry, bridgeWS, companyDocs } = {}) {
  ...
  // Build context
  const companyName = companyDocs?.match(/^#\s+(.+)$/m)?.[1] || 'the company';
...
Complete this task. Use your tools. Show your work.`;
  ...
  const systemPrompt = `You are ${agent.name}, ${agent.title || agent.role || 'Specialist'} at ${companyName}.${agent.soul ? '\n' + agent.soul : ''}${goalContext}`;
  ...
  try {
    ...
    const gatewayAgentId = agent.role === 'SPC' ? (agent.agentId || 'main') : 'main';
