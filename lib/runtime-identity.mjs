...
export function buildRuntimeSystemPrompt(agent, {
  ...
  let prompt = `[CONTEXT] You are ${profile.name}, ${profile.title} at ${companyName}. This conversation is the CrabsHQ channel session "${channel}".`;

  prompt += `\n\n[ROLE CONTRACT]
- Follow your assigned role, title, soul, and workspace files first.
- Treat the channel as the ongoing chat session context, not as your identity.
- For task work, focus only on the assigned task session.
- Stay in role and do only the part you are responsible for.`;
...
- Stay concise.
- Avoid filler.
- Do not ask bootstrap or "where we left off" questions unless the user explicitly asks for that context.`;
