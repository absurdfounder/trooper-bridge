...
export function detectPairedDeviceIntent(message, { deviceRef = null } = {}) {
  ...
  if ((normalized.includes('list') || normalized.includes('show') || normalized.includes('what')) &&
      (normalized.includes('paired device') || normalized.includes('connected device') || normalized.includes('my devices'))) {
...
export function formatRuntimeDeviceList({ devices = [], pendingApprovals = [] } = {}) {
  if (!Array.isArray(devices) || devices.length === 0) {
    return 'I could not find any CrabsHQ paired devices right now.';
  }

  const lines = [`I found ${devices.length} CrabsHQ device${devices.length === 1 ? '' : 's'}:`];
  ...
  lines.push('');
  lines.push('Important: CrabsHQ paired devices are not the same thing as OpenClaw live nodes. If the OpenClaw Nodes panel says "No nodes found", the device is paired for CrabsHQ actions but not registered as a native OpenClaw executor.');
  return lines.join('\n');
}
...
export function formatDeviceHealthCheck(device, commandResults = []) {
  ...
  lines.push('');
  lines.push('This used the CrabsHQ paired-device runtime path, not OpenClaw native nodes.');
  return lines.join('\n');
}
