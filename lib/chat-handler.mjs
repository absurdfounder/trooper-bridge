...
export async function handleChatMessage(msg, user, ws, ctx) {
  ...
  const gatewayAgentId = isSPC ? (agent.agentId || 'main') : 'main';
  ...
  try {
    ...
    try {
      ...
      // Fallback: if the native SPC agent doesn't exist in gateway config yet, retry via 'main'
      if (isSPC && /unknown agent id/i.test(spcErr.message || '')) {
        ...
        const fallbackSessionKey = `agent:main:hook:crabhq:${agentSlugForSession}:channel:${channel}`;
        const fallbackPrompt = systemPrompt + '\n\n[SPC FALLBACK]\nThe gateway does not currently expose the native SPC agent. Respond as the specialist persona requested above, not as the team lead.';
        responseText = '';
        chunkBuffer = '';
        result = await gateway.runAgentStreaming(
          userMessage,
