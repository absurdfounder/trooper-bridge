...
export async function handleChatMessage(msg, user, ws, ctx) {
  ...
  try {
    ...
    try {
      ...
      // Fallback: if the consolidated 'spc' agent doesn't exist in gateway config, retry via 'main'
      if (gatewayAgentId === 'spc' && /unknown agent id\s+"?spc"?/i.test(spcErr.message || '')) {
        captureLog('warn', `SPC agent missing in gateway config; retrying ${agent.name} chat via main agent fallback`);
        const fallbackSessionKey = `agent:main:hook:crabhq:${agentSlugForSession}:chat`;
        const fallbackPrompt = systemPrompt + '\n\n[SPC FALLBACK]\nThe gateway does not currently expose the unified spc agent. Respond as the specialist persona requested above, not as the team lead.';
        responseText = '';
        chunkBuffer = '';
        result = await gateway.runAgentStreaming(
          userMessage,
          { agentId: 'main', agentName: agent.name, sessionKey: fallbackSessionKey, extraSystemPrompt: fallbackPrompt, timeoutMs: 180000 },
          streamCallback,
        );
      } else {
        throw spcErr;
