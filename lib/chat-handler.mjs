...
export async function handleChatMessage(msg, user, ws, ctx) {
  ...
  // Local inference: client handles LLM — bridge only saves/broadcasts the human message
  if (msg.localInference) {
    captureLog('info', `[chat-handler] Local inference mode — skipping gateway for message ${messageId}`);
    return;
  }
